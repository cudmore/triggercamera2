...
logger.debug('triggercamera body')
'''
...
class TriggerCamera(threading.Thread):
	def __init__(self):
		...
		#logger.warning('triggercamera constructor')
		...
		if self.config['triggers']['useTwoTriggerPins']:
			...
			GPIO.setup(self.config['triggers']['triggerpin'], GPIO.IN)	#
			GPIO.setup(self.config['triggers']['framepin'], GPIO.IN)	#
	...
	def ParseConfigFile(self):
		...
		if not configLoaded:
			print 'error: triggercamera loading config file config.ini'
		Config.sections()
	...
	def startVideo(self, thisTrial=None, savePath=''):
		'''
		...
		if self.isArmed and not self.videoStarted:
			...
			self.logFileRoot = sessionStr + self.savename + '_t' + str(self.trialNumber)
